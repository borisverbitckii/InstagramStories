...
extension UsersDataSource: UsersDataSourceProtocol {
    ...
    func fetchUsersFromBD(userType: UserType,
        ...
        let settings = UsersDataBaseManagerSettings(
            ...
            sortingSettings: SortingSettings(sortingKeyPath: "date",
                                             ascending: false),
            predicate: predicate)
        dataBaseManager.executeDBOperation(settings: settings) { (result: Result<[RealmInstagramUserProtocol], Error>) -> () in
            switch result {
    ...
    func fetchUserWithPrimaryKey(primaryKey: Int, completion: @escaping (Result<RealmInstagramUser, Error>) -> ()) {
        ...
        dataBaseManager.executeDBOperation(settings: settings) { (result: Result<RealmInstagramUser, Error>) -> () in
            switch result {
    ...
    func getUserFromBD(primaryKey: Int, completion: @escaping (RealmInstagramUser) -> ()) {
        let settings = UsersDataBaseManagerSettings(type: .getUserWithPrimaryKey, primaryKey: primaryKey)
        dataBaseManager.executeDBOperation(settings: settings) { (result: Result<RealmInstagramUser, Error>) -> () in
            switch result {
            case .success(let user):
    ...
    func saveUserToBD(user: RealmInstagramUserProtocol, isSavedCompletion: @escaping (Bool) -> ()) {
        let settings = UsersDataBaseManagerSettings(
            type: .save,
            user: user)
        dataBaseManager.executeDBOperation(settings: settings) { (result: Result<Bool, Error>) -> () in
            switch result {
